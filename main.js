...
async function microsoft(request, email) {
    try {
        ...
        return { success: false, code: 'microsoft :' + e.message };
    }
};
...
async function randomUserName() {
    ...
    let responseHTML = await baseRequest(options).catch(() => { });
    const document = new JSDOM(responseHTML).window.document;
    return document.getElementById('copy3').innerHTML;
};
...
async function run() {
    try {
        ...
        for (const iterator of domain) {
            const mail = userName + iterator;
            const proxy = getProxy();
            let result = await checkAccount(mail, proxy);
            if (result.info.success) {
                if (!fs.existsSync('success.txt')) {
                    var createStream = fs.createWriteStream("success.txt");
                    createStream.end();
                }
                fs.appendFile('success.txt', mail + '\n', function (err, result) {
                });
            }
            indexResult++;
